...
$("#btnConsultar").click(function() {   
	...
	$.get(
			base_url+'reportes/flujopagosefectuado/getFlujoRangoCuotas/'+$("#proyectos").val()	
	...
	var tr = $(document.createElement('tr'));	
	$.get(
			base_url+'reportes/flujopagosproyectado/getFlujoPagosProyectados/'+$("#proyectos").val()		
		)
		.done(function(data)
		{			
				...
				for(xx = 0;xx<cantidadMeses;xx++ )
				{
					...
					{																		
						// Si coincide la fecha de la reserva con la del pago suma el monto de reserva al total guardado
							...
							total = parseFloat(newArray[xx]) + parseFloat(linea.pagocalculado);
							//total = parseFloat(newArray[cont]);
							monto = parseFloat(linea.pagocalculado);
						...
						/*	monto = parseFloat(linea.pagocalculado) + parseFloat(linea.reserva);
						}
						else{
