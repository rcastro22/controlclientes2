...
$(document).on('click','#btnAgregar',function()
{
	...
	else
	{
			...
			{
				newArray.push({ idaporte: $("#idaporte").val(), idformapago: varCodFormaPago, formapago: varDescripcionFormapago, nodocumento: varNodocumento, monto: varMonto, observaciones: varObservaciones, status: "AC", fechapago: varfechapago });
...
function existeProducto(array)
{
	...
	for(i=0;i<length;i++)
	{
		if(array[i].idformapago==$('#formapago').val() && array[i].idaporte==$('#idaporte').val())
		{
			existe=true;
			break;
		}
...
{	
	var varidaporte = obtenerValorCol(this, "idaporte");
    ...
    {
        if (obj.idaporte == varidaporte && obj.idformapago == varCodFormaPago)
        {
            return false;
        }
        return true;
 ...
 function grabarNuevoPago()
{
	...
	tipopag = $('input:radio[name=optradio]:checked').val();

	$.ajax({
	  type: 'POST',
	  url: base_url + 'movimientos/aporte/grabarNuevoPago',
	  data: {arreglo: newArray, idaporte : $('#idaporte').val(), monto: varTotal, tipopago: tipopag},
	  success: function (msg) {
	  	if(msg != ""){
	  		...
	  		window.location=base_url+"movimientos/aporte/listado/"+$("#idaporte").val();
	  	}
	  }
